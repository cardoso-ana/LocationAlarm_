...
class MapViewController: UIViewController, MKMapViewDelegate, CLLocationManagerDelegate, MPMediaPickerControllerDelegate
{
  ...
  func activateByQuickAction(quickActionType: String)
  {
    ...
    //MARK: Possível solução para recuperar a música atual
    let defaults = NSUserDefaults.standardUserDefaults()
    
    if let savedMusic = defaults.objectForKey("músicaAtual") as? NSData {
    self.mediaItem = NSKeyedUnarchiver.unarchiveObjectWithData(savedMusic) as? MPMediaItem
    }
  ...
  func mediaPicker(mediaPicker: MPMediaPickerController, didPickMediaItems  mediaItems:MPMediaItemCollection) -> Void
  {
    ...
    //MARK: possível solução para salvar musica atual no userdefaults?
    
    let savedData = NSKeyedArchiver.archivedDataWithRootObject(mediaItem!)
    let defaults = NSUserDefaults.standardUserDefaults()
    defaults.setObject(savedData, forKey: "músicaAtual")
