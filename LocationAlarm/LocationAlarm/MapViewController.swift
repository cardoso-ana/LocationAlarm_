...
class MapViewController: UIViewController, MKMapViewDelegate, LocationServiceDelegate, MPMediaPickerControllerDelegate, WCSessionDelegate
{
  ...
  @IBOutlet weak var labelCaptionDistanceToAlarm: UILabel!
  ...
  override func viewDidLoad()
  {
    ...
    searchBar.sizeToFit()
    searchBar.placeholder = "Search Location".localized
    ...
    activeButton.setTitle("ACTIVATE".localized, forState: UIControlState.Normal)
    ...
    self.tutorialLabel.text = "Tap on the map".localized
    self.labelCaptionDistanceToAlarm.text = "distance to alarm".localized
    ...
    if pinAlarm
    {
      ...
      if !alarmeAtivado
      {
        ...
        if distanciaParaRegiao > 0
        {
          ...
          let alarmeAtual = Alarm(coordinate: raioAlarme!.coordinate, radius: raioAlarme!.radius, identifier: uniqueIdentifier, note: String(format: NSLocalizedString("You are X from your destination!", comment: ""), (formataDistânciaParaRegião(raioAlarme!.radius))))
        ...
        {
          showSimpleAlertWithTitle("", message: "You are already".localized, viewController: self)
        }
  ...
  func changeDisplayActivated()
  {
    ...
    tutorialLabel.text = String(format: NSLocalizedString("radius of ", comment: ""), (radiusLabel.text!))
    tutorialLabel.hidden = false
    
    activeButton.setTitle("DEACTIVATE".localized, forState: UIControlState.Normal)
